...
// Declare tableData as a global variable
declare const tableData: TableData;
 ...
 */
function copyRowData(rowIndex: number): void {
 ...
 * @param rowIndex - The index of the row to copy
 */
function copyRowDataAsCSV(rowIndex: number): void {
    if (!tableData || !tableData.rows || rowIndex >= tableData.rows.length) {
        console.error('Invalid row index or missing data');
        return;
    }
 ...
 */
function showCopyNotification(text: string): void {
